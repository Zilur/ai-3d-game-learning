"""CI-only download of a pinned official Godot asset with SHA-256 validation."""
...
def main() -> None:
    if not os.environ.get("RUNNER_TEMP"):
        raise RuntimeError("CI-only installer. Install Godot normally on your computer.")
    ...
    if len(assets) != 1:
        raise RuntimeError("Expected exactly one pinned official asset")
    asset = assets[0]
    digest = asset.get("digest", "")
    if not digest.startswith("sha256:"):
        raise RuntimeError("Missing official SHA-256 digest")
    with tempfile.TemporaryDirectory(dir=destination) as temporary:
        ...
        if actual != digest.removeprefix("sha256:"):
            raise RuntimeError("Godot checksum mismatch")
