...
def main() -> None:
    ...
    parser.add_argument('--objectives', type=Path, default=ROOT/'assessments/learning-objectives.json')
    parser.add_argument('--output', type=Path)
    parser.add_argument('--self-test', action='store_true')
    args = parser.parse_args()
    if args.self_test:
        self_test(); return
    require(args.report is not None, 'provide a report path or --self-test')
    require(args.report.stat().st_size <= 1_000_000, 'report is too large')
    report = json.loads(args.report.read_text(encoding='utf-8'))
    objectives = json.loads(args.objectives.read_text(encoding='utf-8'))['objectives']
    result = render(report, objectives)
    if args.output:
        require(args.output.resolve() not in {args.report.resolve(), args.objectives.resolve()}, 'do not overwrite input')
...
if __name__ == '__main__':
    try:
        ...
        print('LEARNING REPORT ERROR: ' + str(exc), file=sys.stderr)
        raise SystemExit(1)
