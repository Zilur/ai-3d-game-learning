...
with sync_playwright() as p:
 b=p.chromium.launch(executable_path=(__import__('shutil').which('chromium') or __import__('os').environ.get('CHROMIUM_BINARY')),headless=True,args=['--no-sandbox'])
 page=b.new_page(viewport={'width':1180,'height':1000})
 errors=[];page.on('pageerror',lambda e:errors.append(str(e)))
 page.set_content((ROOT/'practice/web/space-lab.html').read_text(encoding='utf-8'))
