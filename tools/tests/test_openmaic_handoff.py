...
from lib import private_context as h
...
class HandoffTests(unittest.TestCase):
    ...
    def test_public_input_does_not_read_private_history(self):
        from lib.course import build_input
        with patch.object(f, 'load_state', side_effect=AssertionError('private read')):
            text, _ = build_input('A04')
        self.assertNotIn('self_reported_notes',text)
        self.assertNotIn('current_lesson_gaps',text)
        self.assertIn('教师',text)
