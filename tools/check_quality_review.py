...
def main():
    ...
    for ident, row in by_id.items():
        ...
        if ident == 'E06':
            ...
            assert '### E06-D2' not in teacher and '### 对话4' not in dialogue
        else:
    ...
    assert 'Origin' in by_id['A07']['questions'][3][1]
    assert 'Quaternion' not in by_id['A07']['questions'][3][1]
    assert '还没制作拾取计数' in by_id['B05']['questions'][2][1]
    assert '到B06/B08再补' in builder.ROWS['B05']['regress']
    for folder in ('curriculum/lessons', 'curriculum/dialogues', 'openmaic/lessons'):
        b09 = (ROOT / folder / 'B05.md').read_text(encoding='utf-8')
        assert '核对已有包装、形状和标记仍保留' in b09
        assert '验证包装的碰撞与拾取仍存在' not in b09
        assert '更新静态外观并保留当前包装职责' in b09
    assert '形状、比例、色彩、光照、表面五栏' in (ROOT / 'curriculum/dialogues/A01.md').read_text(encoding='utf-8')
    assert '窗口' in review.REVIEWS['B08']['proof'][1]
    assert review.REVIEWS['B08']['target'] == 1
    assert '同步重入' in by_id['B06']['failure']
    assert '当前范围内目标' in review.REVIEWS['D06']['diagnostic'][1]
    ...
    mutations = []
    bad = copy.deepcopy(review.REVIEWS); bad['B04']['proof'].pop(); mutations.append(bad)
    bad = copy.deepcopy(review.REVIEWS); bad['B04']['target'] = 3; mutations.append(bad)
    bad = copy.deepcopy(review.REVIEWS); bad.pop('C02'); mutations.append(bad)
    bad = copy.deepcopy(review.REVIEWS); bad['E06']['proof'] = ['illegal software gate']; mutations.append(bad)
    bad = copy.deepcopy(review.REVIEWS); bad['A05']['diagnostic'] = bad['A04']['diagnostic']; mutations.append(bad)
    bad = copy.deepcopy(review.REVIEWS); bad['A05']['ai'] = ''; mutations.append(bad)
