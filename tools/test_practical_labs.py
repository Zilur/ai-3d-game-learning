...
from check_practical_labs import validate_mesh, validate_bindings, ROOT

class Readiness(unittest.TestCase):
    def setUp(self):
        self.mesh = runpy.run_path(str(ROOT/'tools/make_practice_robot.py'))['build']()
        self.bindings = runpy.run_path(str(ROOT/'curriculum/authoring/novice_learning.py'))['BINDINGS']
        self.order = runpy.run_path(str(ROOT/'tools/course_order.py'))['ORDER']
    def test_valid(self):
    ...
    def test_no_hidden_setup_requirement(self):
        self.bindings['C11'] = (['game/labs/absent_file.tscn'],'first','limit','P1')
        with self.assertRaises(ValueError): validate_bindings(self.bindings,self.order)
    def test_path_escape(self):
        self.bindings['A04'] = (['../README.md'],'first','limit','P1')
        with self.assertRaises(ValueError): validate_bindings(self.bindings,self.order)
    def test_k_boundary(self):
        self.bindings['E06'] = (['game/scenes/main.tscn'],'first','limit','P1')
        with self.assertRaises(ValueError): validate_bindings(self.bindings,self.order)
