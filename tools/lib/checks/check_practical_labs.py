...
def validate_bindings(bindings, order, root=ROOT):
    ...
    for ident, binding in bindings.items():
        ...
        for name in binding[0]:
            p = Path(name)
            require(not p.is_absolute() and '..' not in p.parts and name.startswith(('practice/godot/','practice/blender/','practice/web/')), 'unsafe resource path')
...
def main():
    ...
    _obj, _cards, bindings, order = catalog(ROOT)
    validate_bindings(bindings, order)
    original = json.loads((ROOT/'practice/godot/assets/practice_robot.gltf').read_text())
    validate_mesh(original)
    expected = runpy.run_path(str(ROOT/'tools/assets/make_practice_robot.py'))['build']()
    ...
    glb = (ROOT/'practice/godot/assets/practice_robot_roundtrip.glb').read_bytes()
    ...
    for name in ('transform_origin_lab.blend','kitbash_style_lab.blend','uv_material_lab.blend','animation_fixture.blend'):
        require((ROOT/'practice/blender'/name).stat().st_size > 10_000, 'missing/empty ready blend')
    require('practice/README.md' in (ROOT/'README.md').read_text(), 'missing main entry')
    require('lab_hub.tscn' in (ROOT/'practice/godot/scripts/main.gd').read_text(), 'missing in-game entry')
    require('--godot' in (ROOT/'.github/workflows/check.yml').read_text(), 'runtime assertions not in CI')
