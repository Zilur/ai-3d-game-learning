...
def main() -> None:
    folder = ROOT / 'practice/godot/assets/village'
    ...
    generator = runpy.run_path(str(ROOT / 'tools/assets/build_world_assets.py'))
    ...
    from ..course import catalog
    bindings = catalog(ROOT)[2]
    expected = {'E01': 'navigation_lab', 'E03': 'shader_lab', 'E05': 'large_scene_lab', 'C08': 'effects_lab'}
    for ident, stem in expected.items():
        require('practice/godot/labs/' + stem + '.tscn' in bindings[ident][0], ident + ' missing real lab route')
        for suffix in ('.tscn', '.gd'):
            require((ROOT / ('practice/godot/labs/' + stem + suffix)).is_file(), stem + ' missing file')
    require('practice/godot/world/exploration.tscn' in bindings['D07'][0], 'D07 missing integrated game')
    require(not bindings['E06'][0], 'K-only E06 must not gain software requirement')
    require('run/main_scene="res://world/exploration.tscn"' in (ROOT / 'practice/godot/project.godot').read_text(), 'new main not selected')
    for name in ('practice/README.md', 'docs/maintenance.md', 'practice/godot/shaders/learning_surface.gdshader',
                 'practice/godot/world/stream_piece.tscn', 'practice/godot/tests/production_suite.gd', 'practice/godot/tests/save_roundtrip.gd'):
        require((ROOT / name).is_file(), 'missing production source: ' + name)
    require((ROOT / 'practice/godot/scenes/main.tscn').is_file(), 'old reference must remain')
