...
|web/space-lab.html|本课程离线交互示例|网页演示不能替代Godot物理和渲染结果|

没有新增下载的第三方字体、模型、音频或图片。实际引入第三方素材时另记文件、作者、原始来源、取得日期、许可与修改内容；不能仅写“AI生成所以不用查”。

当前没有为整个仓库自动选择新的发布许可证。仓库公开、可以访问源码与商用/再分发授权是不同事项；发布前由仓库主人决定并补LICENSE。不要把此记录当成对所有未来素材的法律保证。
