...
从主游戏进入实验可以暂存接续，右上角“继续刚才的游戏”回到区域安全点；暂存不是磁盘保存。

实验目录共17个入口，含已有参考与A03起点；不是新增17套独立课程。每课只用0A指向的一个模式，先预测，再改一项。新实验的中文侧栏可滚动、可键盘聚焦；旧参考界面中保留的英文可对照逐课说明，不要求背按钮名。

**孩子不需要先写脚本、搭完整场景、创建测试骨架或下载角色。** 实验脚本在运行时装配的节点，可在运行后的Remote场景树看见；编辑器启动前看到一个根节点不表示场景没做完。要保留修改请用`tools/workspace.py create my-world`创建自己的副本；`world/creation.tres`与StarLayout标记可保存。Remote临时修改不是保存到原场景。
