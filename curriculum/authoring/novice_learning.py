...
BINDINGS = {'A01': ([], '从自己提供的参考中选一个可见特征，说出借鉴与不借鉴什么。', '参考分析，无软件Lab门槛；没有图不捏造作品内容。', 'P0'),
 ...
 'C01': (['game/scenes/main.tscn'], '先画主路与返回路径，用代理块在副本扩一个区域。', '当前主场景不是已完成的庭院/林路/观景台三段地图。', 'P1/P7'),
 'C02': (['game/scenes/visual_lab.tscn'], '只切换Clutter，指出焦点、路径和留白变化。', '密度样例；不包含全套视差/背景层实现。', 'P4/P8'),
 ...
 'C07': (['blender/labs/uv_material_lab.blend', 'blender/labs/kitbash_style_lab.blend'],
         ...
         'P6'),
 'C08': (['game/scenes/visual_lab.tscn', 'game/labs/material_lab.tscn', 'game/labs/camera_lab.tscn'],
         '在现成同机位样本中先比较光照、材质或投影的一项，再核对实际渲染器。',
         '提供实际渲染基线；Fog/Glow/DOF等候选仍按目标渲染器另试，不假装全套效果控件已配齐。',
         'P4/P9'),
 'C09': (['game/scenes/main.tscn'], '对同一构建/机位选择一项真实测量，不改多种设置。', '基准由目标设备提供，headless不代表GPU画面性能。', 'P7/P11'),
 'C10': (['game/labs/interaction_lab.tscn'],
         '拖动角色接近蓝区，先不拿钥匙尝试开门，再只改变钥匙条件。',
         '真实检测/允许条件/门状态/提示分离，重复开门可验证；不含背包或完整正式关卡系统。',
         'P2/P7'),
 'C11': (['game/labs/interaction_lab.tscn'],
         '在门实验先保存门进度与偏好，再改临时位置、门或偏好，按读取观察哪些恢复。',
         '真实专用本地文件，可准备缺失/坏档/v0旧档；恢复初值不删文件，只能删除本实验档，不代表正式游戏存档已接入。',
         'P2/P11'),
 'C12': (['game/scenes/main.tscn'], '核对目标平台和当前导出条件，再生成自己的可运行包。', '有项目源码不等于已给出跨平台成品包。', 'P11'),
 ...
 'D04': (['blender/labs/kitbash_style_lab.blend', 'game/scenes/main.tscn'],
         '挑一个小区域组合已合格部件，固定机位做减法。',
         '当前文件只是起点，不是已验收美术结业作品。',
         'P8'),
 'D05': (['blender/labs/animation_fixture.blend', 'game/labs/animation_lab.tscn'],
         ...
         'P4/P10'),
 'D06': (['game/labs/animation_lab.tscn', 'game/scenes/main.tscn'],
         '先播放一次攻击并中断，比较目标在内/在外与重复输入下的命中次数。',
         '现成窗口/距离/一次性规则实验；未把攻击接入主游戏控制器，关闭训练角后的十星回归仍要自己集成后验证。',
         'P10'),
 'D07': (['game/scenes/main.tscn'], '按已选范围检查自己的完整构建和真实试玩，不增新玩法。', '现有参考不代表精致三段地图已完成。', 'P11'),
 'E01': ([], '有真实寻路需要时，先分清可画路径与角色能通过。', '导航选修无专用Lab，不阻塞基础版。', '按需专项'),
 'E02': (['game/scenes/main.tscn'], '先固定实际设备与构建，定义要解决的一个性能问题。', '参考场景不是通用性能测试集。', 'P11'),
