...
遵循[项目主线](curriculum/project-spine.md)和[单课项目衔接模板](openmaic/project-integration-template.md)。概念先在可重置小实验理解，再把已验收增量接入同一个小庭院。项目进展可以是功能、视觉、手感、可靠性或判断证据，不必每课加新玩法；高级选修不强行进入主项目。

逐课回填时明确：起始状态、预期效果、AI学习与工作指令、允许/禁止修改范围、少量人工微调入口、接入位置、相关旧功能回归。没有真实软件连接不能声称已执行；自动化测试和学员掌握分开记录。批量AI能力的可靠性需按实际模型/工具复测，不预言永久弱点。

本轮只新增衔接文档与模板，不代表45份课件已逐一回填新的AI协作卡，也不代表ABCDE新编号已全部迁移。后续重编编号须保存旧新映射；生成器与源、学生稿、教师稿、索引和清单需同时同步，不能另建一套互相冲突的题号。
