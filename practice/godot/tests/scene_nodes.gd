...
func _check(condition: bool, message: String) -> void:
	...
	if not condition:
		...
		push_error("SCENE NODES FAILED: " + message)

func _run() -> void:
	for filename in ["starter", "reference", "broken"]:
		var packed := load("res://labs/scene_nodes/%s.tscn" % filename) as PackedScene
		...
		_check(module.scene_file_path == "res://labs/scene_nodes/module.tscn", "reuses a scene, not loose copied nodes")
	...
	if failures.is_empty():
		print("SCENE NODES PASS: %d assertions" % checks)
		quit(0)
	else:
		print("SCENE NODES FAILED: %d of %d" % [failures.size(), checks])
		quit(1)
